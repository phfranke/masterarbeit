...
global {
	...
	float dichtekoeffizient parameter: "Koeffizient Dichteabhängige Sterblichkeit: " category: "Überlebensrate" init:0.01;
...
species nachwuchs {
	...
	reflex entwicklung {
		...
		float sterb_larven <- (dichtekoeffizient*(dichte_linear)^2 + (1-ueberlebensrate_larve))/zeit_larve;
...
species migrant skills: [moving]{
	...
	reflex wandern {
		if wandern and (current_date - self.start_wanderung) >= (wanderDauer*3600*24) {
...
//				do wandern;
			...
			//	ueberwintern;
				...
				current_date <- current_date + 60#day;
				}
			}
		}
	}
