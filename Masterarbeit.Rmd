...
Im Larvenstadium wird eine dichteabhängige Sterblichkeitsrate eingeführt. Die Annahme ist, dass mit zunehmender Dichte die Sterblichkeit quadratisch ansteigt. Diese Kurve wird geeicht mittels eines Koeffizienten, dessen Höhe im Kalibrationsprozess bestimmt wird. Die grundlegende Sterblichkeit entspricht der Sterberate bei unbedeutender Larvendichte und wird gestützt auf die Werte aus anderen Stadien  angenommen. Das Resultat aus \@ref(eq:sterblarve) ergibt die Sterblichkeit über die ganze Dauer der Larvalentwicklung. Die Sterblichkeit soll im Modell jedoch täglich angewendet werden, weshalb das Ergebnis durch die Dauer der Larvalentwicklung (`parameter(maxAge_egg)`) geteilt wird. Es gilt dabei:

- $S$=Sterblichkeit pro Tag
- $S_{grund}$ = Grundsterblichkeit
- ${Koeff}$=Koeffizient
- ${Dichte}_{aktuell}$ = aktuelle Larvendichte in $\frac{Anzahl}{m^2}$


\begin{equation}
{S = \frac{S_{grund}+({Koeff}*{Dichte}_{aktuell})^2}{Anzahl Tage}}
(\#eq:sterblarve)
\end{equation}
