...
- die Dauer der Wanderungen
- der Koeffizient der dichteabhängigen Sterblichkeit

Beide Parameter beeinflussen sich gegenseitig vermutlich nicht oder nur gering. Da sie grundsätzlich andere Funktionen haben, welche voneinander abgekoppelt sind, ist es möglich die beiden Parameter zu kalibrieren ohne sämtliche Wertekombinationen auszutesten. Deshalb wird der Dichtekoeffizient in einem ersten Schritt auf einen Wert festgelegt, welcher die stabile Funktionsweise des Modells sicherstellt. Insbesondere müssen genügend wandernde Kammolche entstehen, welche in alle Himmelsrichtungen aus den Weiher wandern. Damit kann sichergestellt werden, dass das Auffinden der Gewässer durch die Wanderdauer beeinflusst wird und nicht durch einen Mangel an Wanderern. Experimentell hat sich ein geeigneter Wert von 0,1 für den Dichtekoeffizient herausgestellt. Nach der Kalibrierung der Wanderdauer wird diese auf den ermittelten Wert fixiert und anschliessend der Dichtekoeffizient kalibriert. 

### Wanderdauer
Als beobachtete Grösse für diese Kalibration wird die Anzahl der besiedelten Gewässer verwendet. Wie im Kapitel \@ref(daten) gezeigt, konnten bisher nur in 5 Gewässern Kammmolche festgestellt werden. ES handelt sich um die Gewässer mit der Indentifikationsnummer BL191, BL195, BL196, BL197 und BL198. Es gilt nun einen Wert für den Parameter der Wanderdauer zu finden, welcher ebenfalls zu einer Besiedelung dieser Gewässern führt. Für die Kalibrierung wurde mit jedem Wert für die Wanderdauer das Modell fünf mal durchgeführt. Als Startdatum dient jeweils der 1.1.2000  und als Enddatum der 1.1. 2022. Die Werte der Kalibrationsdauer wurden experimentell auf den hier verwendeten Wertebereich eingeschränkt. Die Resultate sind in Abbildung \@ref(fig:kaliMigrant) gezeigt. Es zeigt sich, dass sowohl eine Wanderdauer von 30 als auch von 40 Tagen zu einer Besiedelung von 5 Weihern führen kann. Da damit gerechnet werden muss, dass die Kammmolche unendteckt ein Gewässer besiedeln können, wird hier der höhere der beiden Werten als der korrekte Parameterwert gewählt. Für die weiterführenden Arbeiten wird dieser also auf 40 Tage eingestellt. 
...
Für die Kalibrierung werden die Daten aus der Fangaktion 2021 verwendet. Es können drei Aspekte aus den Fangzahlen verwendet werden:
- Anzahl gefangene Tiere (adulte und juvenile)
- Gewichtsverteilung der Tiere (adulte)

Die Gewichtsverteilung ist nur mässig geeignet für die Kalibrierung, da das Gewicht der Kammmmolche nicht modelliert wird. Geht man davon aus, dass die Gewichstzunahme linear erfolgt, kann zwar die Gewichtsverteilung aus den Fangdaten mit der Altersverteilung aus dem Modell verglichen werden. Daraus aber einen geeigneten Parameterwert für die Sterblichkeit der Larven abzuleiten, erscheint nicht sinnvoll.
Für die Kalibrierung des Modells werden nur die Fangdaten aus 2021 berücksichtigt, die Daten von 2022 werden für die Validation benötigt und müssen somit hier ausgeschlossen werden. Die Fangdaten von 2021 haben aber den Makel, das sie wohl nicht vollständig sind. Wie in Kapitel \@ref(daten) beschrieben, wurde die Fangaktion vermutlich erst gestartet als bereits viele Tiere in den Gewässern waren. Deshalb müssen diese Zahlen noch korrigiert werden. Im selben Kapitel wurde gezeigt, dass über die gleiche Zeitspanne aber sehr gut korrespondieren. Deshalb kann davon ausgegangen werden, dass auch die Summe der Tiere ungefähr übereinstimmen muss.

Dennoch wird mit Hilfe der Fangzahlen ein sinnvoller Parameterwert ermittelt.

Wir können davon ausgehen, dass die gefangenen Tiere in den ersten beiden Jahren ungefähr gleich hoch sind, da die Tiere nicht jedes Jahr ans Gewässer kommen.




Im Modell wird dieser Koeffizient durch den Parameter `density_koeff` bestimmt. Für die Kalibrierung deises Parameters wurde das Modell mit jedem Wert 3 mal durchlauffen lassen. Die Auswirkungen des Dichtekoeffizienten sind in Abbildung \@ref(fig:kaliDichte) veranschaulicht. Sowohl die Streuung der Resultate als auch die Anzahl der gefangenen Kammmolche nimmt mit wachsendem Koeffizienten. Wie in Kapitel \@ref(daten) beschrieben, sind die Fangzahlen aus dem ersten Jahr wohl nicht repräsentativ. Die Fangsaison war wahrscheinlich zu kurz und die Anzahl gefangener Individuuen, wäre wohl ungefähr doppelt so hoch ausgefallen, wenn das Abfangen schon früher begonnen hätte. Die Streuung der modellierten Fänge ist auch recht hoch, weshalb das Modell in diesem Fall mit einer schätzenden herangehensweise kalibriert wird.
Als Zielwert für die Kalibrierung wird hier eine Zahl zwischen 500 und 1200 angestrebt. Die Kalibrierungsläufe des Modells liefern für den `Koeffizienten = 0.008` ein gut passendes Resultat. Die Streuung liegt praktisch innerhalb des angestrebten Bereichs. Der Parameter `density_koeff` wird also auf den Wert `0.008` festgelegt.
