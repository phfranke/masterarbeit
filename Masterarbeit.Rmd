...
Der Übergang zwischen den Stadien wird durch den Kalender bestimmt. Die Dauer der einzelnen Entiwcklungsstadien in Tagen wird in Parametern festgelegt. Beim Übergang wird die Sterblichkeitsrate integriert. Die Individuuen entwicklen sich gestützt auf eine Wahrscheinlichkeit weiter oder sterben. Ausnahme ist Spezies "Adult", hier wird eine jährliche Sterblichkeit implementiert.
...
- *isEgg*: true 
- *count*: Anzahl Eier, Gemäss Parameter *clutchSize*
- *nativePond*: angestammtes Brutgewässer, ist gleich dem aktuellen Gewässer, wird auf die Variabel *nativePond* der Mutter gesetzt
- *layDate_egg*: aktuelles Modelldatum
- *location*: aktueller Aufenthaltsort, wird auf die Variabel *location* der Mutter gesetzt

#### Schlüpfen
Beim Schlüpfen werden drei Variabeln verändert:
- *count*: wird anhand der Überlebensrate der Eier berechnet, aktueller Wert mal Parameter *survival_egg*
- *isEgg*: false
-  *hatchDate_larva*: aktuelles Modelldatum
...
- *isFemale*: if random(1.0) >= 0.5: true, else: false
- *dateDevelopment*: aktuelles Modelldatum
...
#### Spezies Nachwuchs
Die ersten beiden Stadien ("Eier" und "Larven") sind an Gewässer gebunden, umfassen viele Individuen und sind von kurzer Lebensdauer. Sie unternehmen keine Wanderungen und erhalten hier lediglich die Aufgabe zu altern und zu sterben. Die einzige Grösse, welche hier transportiert werden muss ist die Anzahl an Überlebenden. Diese Stadien werden deshalb nicht nach einem agentenbasierten Modellansatz modelliert, sondern in einem formelbasierten. Das Modell verfolgt stark vereinfacht die Vorgehensweise, wie sie @WALLENTIN2017165 präsentiert. Agenten der Spezies Nachwuchs stammen von den Muttertieren ab, welche die Eier legen. Iin der ganzen Lebensdauer der Agenten dieser Spezies, werden zwei Arten von Sterblichkeit berechnet. Diese verändern die Variabel "Anzahl" und sind im Detail in den Kapiteln \@ref(dichtesterb) und  REFERENZ AUF KaPITEL STERBLICHKEIT VON EIERN erklärt. Es sind folgende Ereignisse betroffen: 
...
#### Bekämpfung
Ab dem Modelldatum 1.1.2021 werden Bekämpfungsmassnahmen simuliert. In der Natur werden Tiere bei der saisonalen Zuwanderung in die Gewässer mithilfe von fest installierten Zäunen gefangen. Weibchen, welche in der Fortpflanzungspause sind, suche mutmasslich keine Gewässer auf und werden auch nicht gefangen. Gewässer, an welchen Zäune installiert werden, sind mit dem Variabeln *catchingNet* == true markiert. Die Tiere, welche abgefangen werden, werden nach folgenden Kriterien gefiltert:

- *nativePond.catchingNet* == true
- *lastReproductino* + *reproductionBreak* >= current_date
- *slipped* == false

Tiere erhalten eine Chance (Parameter *probabilityCatch*) um durch die Zäune zu schlüpfen. In diesem Fall wird die Variabel *slipped* auf true gesetzt. Diese Variabel wird jedes Jahr wieder auf False gesetzt.
