...
Für die Erhebung der Resultate werden Fangszenarien angenommen, wie weiter oben näher beschrieben (siehe Kapitel \@ref(szenarien)). Das Modell wird auch für die Erhebung der Resultate mehrfach durchgeführt. Für jedes Fangszenario wird das Modell dreimal durchgeführt. Als Resultate wird hier die adulte Kammmolche im gesamten Modell verwendet. In Abbildung \@ref(fig:res) sind diese Resultate dargestellt. Für jedes Jahr ist der Mittelwert (Punkt) und die Spanne (senkrechte Striche) der Anzahl Adulten für die drei Durchläufe dargestellt.

```{r res, echo=FALSE, fig.cap="Entwicklung der adulten Kammmolche unter Berücksichtigung der Fangwahrscheinlichkeit", fig.show="hold", out.width="33%"}
adult_alt1
adult_alt2
adult_alt3
...
Die Kammmolchpopulation konnte hier nicht eingedämmt werden. Die Kurven zeigen sogar erschreckend hohe Werte. Die Analyse der besetzten Gewässer zeigt den Grund. In Abbildung \@ref(fig:karte-bes1) sind die Weiher dargestellt, welche am Ende des Modellzeitraums besetzt waren. Es werden einige Gewässer von Kammmolchen bewohnt, wo keine Abfangtätigkeiten stattfinden. Die Gesamtpopulation kann in diesem Fall recht unbehelligt weiterleben, obwohl an einzelnen Gewässern massiv Tiere abgefangen werden.

```{r karte-bes1,echo=FALSE,out.width="100%",fig.cap="Es sind deutlich mehr Weiher besetzt als angenommen."}
knitr::include_graphics("./bilder/resultate_besetzte_weiher.jpg")
```

Es ist möglich und auch plausibel, dass Weiher unbemerkt besiedelt werden. Die Grössenordnung der überlebenden Molche ist recht extrem. Dass Gewässer mit mehreren Hundert Individuuen unbemerkt bleiben, erscheint als unwahrscheinlich. Bei der genauen Kontrolle der Gewässer anhand deren Beschreibung im [Weiherinventar](https://geoview.bl.ch/?map_x=2607459.1724015&map_y=1265883.2999268&map_zoom=7&tree_group_layers_Natur%20u.%20Landsch.=weiherinventar&tree_groups=Natur%20u.%20Landsch.) fällt auf, dass es einen Weiher gar nicht mehr zu geben scheint. Das Gewässer mit der Nummer "BL900" wird hier beschrieben als "nur noch als Mulde im Gelände wahrzunehmen, ohne Wasser".

Die Daten der Bodenbedeckung werden manipuliert, damit dieses Gewässer nicht als Brutgewässer verwendet werden kann und das Modell erneut durchlaufen gelassen.




Zweiter Durchlauf, alle Teiche fangen!
