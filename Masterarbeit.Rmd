...
- Agenten: In den meisten Fällen entspricht ein Agent im Modell einem Individuum der realen Welt. Es handelt sich um eine Entität einer definierten Spezies. In Spezialfällen kann ein Agent viele Individuuen beinhalten.
...
- Population: Summe aller Individuuen einer Art, welche in der selben Region leben und sich regelmässig vermischen. 
...
- `adult`: Erwachsene, fortpflanzungsfähige Individuuen.
...
create migrant{...}, // innerhalb der geschweiften Klammer werden Variabeln gesetzt
else: isMigrant <- false
```

Die Wanderungen selber verlaufen bei beiden Spezies nach gleichem Muster:
Sie verlassen das Gewässer in einer zufälligen Himmelsrichtung. Bei jedem Schritt weichen sie zufällig +/- 30° vom Kurs ab. Die zurückgelegte Distanz wird auf 5 Meter pro Tag festgesetzt. Wenn der Auslöser für die Rückkehr in die Gewässer anschlägt, wählen die wandernden Agenten dasjenige Gewässer, welches ihnen am nächsten liegt (Luftlinie) als neues angestammtes Gewässer. Dieses wird im Attribut `nativePond` festgelegt. Die Wanderung erfolgt unabhängig vom Untergrund, mit einer Ausnahmen: Tiere können nicht durch Gebäude wandern. Vor jedem Schritt wird vorausgeschaut ob ein Haus im Weg ist und die Wandergeschwindigkeit und Richtung so angepasst, dass die Hindernisse umgangen werden.


#### Bekämpfung
Ab dem Modelldatum 1.1.2021 werden Bekämpfungsmassnahmen simuliert. In der Natur werden Tiere bei der saisonalen Zuwanderung in die Gewässer mithilfe von fest installierten Zäunen gefangen. Kammmolche, welche in der Fortpflanzungspause sind, suchen keine Gewässer auf und können deshalb nicht gefangen werden. Gewässer, an welchen Zäune installiert werden, sind mit der Variabel `catchingNet == true` markiert. Die zuwandernden Kammmolche erhalten eine Chance (Parameter `probabilityCatch`) um durch die Zäune zu schlüpfen. In diesem Fall wird die Variabel `slipped` auf true gesetzt. Diese Variabel wird jedes Jahr wieder auf False gesetzt. Nachfolgend sind die Kriterien aufgelistet, nach welchen zu fangende Tiere selektiert werden:
...
Das Modell versucht äusserst komplexe, natürliche Abläufe abzubilden. Natürliche Prozesse sind nie starr auf einen Parameter ausgerichtet, sondern sind von dutzenden Faktoren abhängig. Das Modell ignoriert bei den meisten Einflussgrössen diese natürliche Bandbreite und legt einen festen Wert dafür fest. Eine Abbildung der Einflussgrössen mit einer zufälligen Schwankung birgt die Gefahr, dass das Modell chaotisch verläuft und die Resultate nicht nachvollziehbar sind. Deshalb basiert das Grundgerüsts des Modells auf starren Abläufen, welche sich nach dem Modelldatum und Parameterwerten richten. Wo immer möglich werden die Parameter anhand Angaben aus der Literatur festgelegt. Es wird bewusst darauf verzichtet, Schwankungsbereiche aus der Literatur mit zufälligen Ereignissen nachzubilden um ein möglichst eindeutiges Modell zu erhalten und chaotisches Verhalten zu verhindern. Dies betrifft zeitliche Entscheidungen, wie die Entwicklungsdauer der einzelnen Stadien, die Anzahl gelegter Eier sowie die Wandergeschwindigkeit. Die Sterblichkeitsrate wird ebenfalls als fester Wert implementiert, die Entscheidung ob ein Individuum stirbt ist hingegen ein zufälliges Ereigniss. Es wird eine Zufallszahl zwischen `0` und `1` generiert. Ist diese Zahl grösser als die Sterblichkeitsrate, stirbt das Individuum. Weitere Entscheidungen sind nicht eindeutig durch die Literatur belegt und beruhen entweder auf Annahmen, werden durch experimente bestimmt (siehe \@ref(kalibration))  oder werden mit Szenarien in das Modell implementiert (siehe Kapitel \@ref(szenarien)). Die Wandergeschwindigkeit basiert auf einer Annahme, die dichteabhängige Sterblichkeit sowie die Dauer der Wanderungen werden kalibriert, die Wanderrichtung basiert auf einem Zufallsereignis und Szenarien werden für Fangwahrscheinlichkeit der Bekämpfung verwendet.


#### Emergenz
Das Modell vereint starre Parameter und zufällige Ereignisse. Dadurch entstehen natürlich wirkende Muster. Die Treiber der Emergenz sind hier die zufällige Wanderrichtung gepaart mit der Sterblichkeit der Adulten. Der Verlauf des Modells ist davon abhängig wie rasch in einem Gewässer Eier abgelegt und dieses somit erfolgreiche besiedelt werden kann. Verirren sich nur einzelne Tiere in ein Gewässer, ist die Wahrscheinlichkeit, dass diese sterben bevor sie sich fortpflanzen können recht gross. Sobald aber zur Brutzeit ein Männchen und ein Weibchen im gleichen Gewässer zugegen sind, wird dieses aller Wahrscheinlichkeit nach dauerhaft besiedelt.

#### Sensing
Der einzige Prozess in dem die Agenten mit der Umwelt interagieren ist beim Berechnen von Wanderhindernissen. Die wandernden Agenten überprüfen bei jedem Schritt ob ein Hinderniss (namentlich Gebäude) auf ihrem Weg liegt.


#### Interaction
In zwei Prozessen geschehen Interaktionen zwischen mehreren Agenten: 

- die dichteabhängige Sterblichkeit bei Larven basiert auf der Anzahl Larven im gleichen Gewässer. Das Überleben eines Individuum ist hier direkt abhängig davon, ob und wieviele andere Larven sich im Gewässer befinden und wie gross das Gewässer ist.
...
Beide Parameter beeinflussen sich gegenseitig nicht wesentlich, da sie grundsätzlich unterschiedliche Funktionen im Modell übernehmen. Für die Kalibrierung der Wanderdauer wird ein Wert für den Dichtekoeffizienten angenommen. Dieser Wert muss sicherstellen, dass genügend Kammmolche in alle Himmelsrichtungen wandern. Damit kann sichergestellt werden, dass das Auffinden der Gewässer durch die Wanderdauer beeinflusst wird und nicht durch einen Mangel an wandernden Individuuen. Experimentell hat sich ein geeigneter Wert von 0,1 für den Dichtekoeffizient herausgestellt. Nach der Kalibrierung der Wanderdauer wird diese auf den ermittelten Wert fixiert und anschliessend der Dichtekoeffizient kalibriert.
...
Im Modell wird dieser Koeffizient durch den Parameter `density_koeff` bestimmt. Für die Kalibrierung dieses Parameters wurde das Modell mit unterschieldichen Werten jeweils 3 Mal durchgeführt. Der verwendete Wertebereich (`r koeff_levels`) wurde experimentel ermittelt. Die Auswirkungen des Dichtekoeffizienten sind in Abbildung \@ref(fig:kaliDichte) veranschaulicht. Es ist ersichtlich, dass sowohl die Streuung der Resultate als auch die absolute Anzahl der gefangenen Kammmolche mit grösserem Koeffizienten abnimmt. 
Wie in Kapitel \@ref(daten) beschrieben, sind die Fangzahlen aus dem ersten Jahr wohl nicht repräsentativ. Die Fangsaison war mutmasslich zu kurz und die Anzahl gefangener Individuuen wäre wohl ungefähr doppelt so hoch ausgefallen, wenn das Abfangen schon früher begonnen hätte. Die Streuung der modellierten Fänge ist ebenfalls recht hoch, weshalb das Modell in diesem Fall mit einer schätzenden herangehensweise kalibriert wird.
Als Zielwert für die Kalibrierung wird hier eine Zahl zwischen 700 und 1200 angestrebt. Die Kalibrierungsläufe des Modells liefern für den `Koeffizienten = 0.008` ein gut passendes Resultat. Die Streuung liegt praktisch innerhalb des angestrebten Bereichs. Der Parameter `density_koeff` wird also auf den Wert `0.008` festgelegt.
