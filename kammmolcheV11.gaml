...
global {
	...
	init{
		create bb from:shp_bb with:[
			...
			aussetzungsort::bool(read("aussetzung")),
			flaeche::float(read("flaeche_m")),
			bekaempfung::bool(read("bekaempfun"))
		];
		create kammmolch number:anz_km_start {
			self.location <- any_location_in(one_of(where (bb,each.aussetzungsort)));
...
experiment kalibration type:batch repeat:1 until:current_date=date("20200101") {
