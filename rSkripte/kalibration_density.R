...
data_graph_kali_dens <- data.frame(koeff_levels,min,max,mean)
...
graph_density <- ggplot(data_graph_kali_dens, aes(koeff_levels,mean,group=koeff_levels)) +
  geom_point() +
  geom_errorbar(aes(ymin=min,ymax=max),width=0.0002)+
