...
Das Modell bildet die Stadien nach, wie sie Kammmolche während ihrer Entwicklung durchleben. Im Modell werden für diese Stadien drei Spezies angelegt, eine weitere Spezies wird für die Wanderungen benötigt. In der Grafik in Abbildung \@ref(fig:grafik-spezies) sind diese Spezies, ihre Variabeln und die Prozesse abgebildet. Es handelt sich um eine vereinfachte Darstellung, welche in den nächsten Abschnitten näher beschrieben wird.

``` {r grafik-spezies, out.with="60%", fig.cap="Verreinfachte, grafische Darstellung des Modells"}
knitr::include_graphics('./bilder/grafik_spezies.jpg')
```

Die Stadien `juvenil` und `adult` erhalten eigene Spezies, die Stadien `egg` und `larva` hingegen werden in Superspezies `offspring` zusammengefasst. Die Spezies `migrant` dient der Modellierung der Wanderbewegungen und kann Agenten der Spezies `juvenil` und `adult` aufnehmen. Zwei weitere, hier nicht angebildete Spezies werden im Modell für die modellierung der Umwelt verwendet. `groundCover` ist für die Darstellung der Bodenbedeckung zuständig und dient der Berechnung von Wanderhindernissen, `pond` enthält sämtliche potentiellen Laichgewässer. Der Übergang zwischen den Stadien ist in Kapitel \@ref(prozess) detailliert beschrieben.
...
Die Struktur der Variabeln ähnelt sich in den Spezies. Es gibt in jeder Spezies eine Datumsvariabel, welche das Datum enthält, an welchen der Agenten geschaffen wurde. Die Variabel `nativePond` enthält einen  Verweis auf das angestammte Gewässer und hat ist ein Fremdschlüssel auf die Spezies `pond`. Dadurch kann direkt von einem Individum auf Variabeln seines Laichgewässers zugegriffen werden. Das Geschlecht des Agenten wird durch `isFemale` gespeichert. Weitere Variabeln werden für die Steuerung von Prozessen benötigt. `location` wiederum enthält die Koordinaten des aktuellen Aufenthaltsortes. Bei Variabeln, welche nur zwei unterschiedlichen Werte annehmen können, wird jeweils ein Boolscher Datentyp (`true`/`false`) verwendet. Dies vereinfacht die Filterung der entsprechenden Individuen, ist einfacher zu prozessieren und weniger fehleranfällig. Die Bezeichnung der betreffenden Variabeln zeigt welche Bedeutung die Variabel annimmt, wenn sie `true` enthält. Der andere mögliche Wert wird demnach durch `false` repräsentiert. Dies betrifft die Variabeln `isEgg` (`true`=Stadium Ei, `false`=Stadium Larve), `isJuvenil` (`true`=Stadium Juvenil, `false`=Stadium Adult), `isFemale` (`true`=weiblich, `false`=männlich) und `slipped` (`true`=entwischt `false`=gefangen).

Hier eine Übersicht über alle Variabeln:
...
- `nativePond` = Vererbung der Variabel `nativePond` der Mutter
- `layDate_egg` = `current_date`
- `location` = Vererbung der Variabel `location` der Mutter

#### Schlüpfen {#hatch}

Der Übergang zwischen dem Stadium Ei und Larve erfolgt beim Schlüpfen. Nur `50%` der Eier erreichen das Larvenstadium. Diese natürliche Sterblichkeit wird hier in der Veränderung der Variabel `count` angewendet. Der Übergang zwischen den Stadien wird durch die Veränderung der Variabel `isEgg` von `true` auf `false` vollzogen. Der Zeitpunkt des Schlüpfens wird durch die Variabel `layDate_egg` und den Parameter `maxAge_Egg` bestimmt. Es werden folgende drei Variabeln in der Spezies `offspring` verändert:
...
- `nativePond` = Vererbung der Variabel `nativePond` des kreierenden Agenten
- `location` = Vererbung der Variabel `location` des kreierenden Agenten
- `isFemale` = Vererbung der Variabel `isFemale` des kreierenden Agenten
