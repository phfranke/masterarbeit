...
``` {r grafik-spezies, out.with="80%", fig.cap="Vereinfachte, grafische Darstellung der im Modell angelegten Spezien und Prozesse"}
...
```{r boxplotweight, echo=FALSE, fig.cap="Gewicht der gefangenen, adulten Tiere", out.width='80%'}
...
```{r kaliMigrant, echo=FALSE, out.width='100%',fig.cap="Ergebnisse der Kalibration."}
ggplot(data_graph_kali_migr, aes(time,mean)) +
  geom_point(aes(color="Mittelwert")) +
  ...
  scale_color_manual(name="Legende",values=c("black","black","grey")) + 
  theme_bw() +
  ...
  geom_smooth(method="lm",formula=y ~ x, se=FALSE,aes(color="Regressionosgerade"), show.legend=TRUE) +
  guides(colour=guide_legend(override.aes=list(linetype=c("solid","blank","solid"),shape=c(NA,16,NA))))
...
ggplot(data_graph_kali_dens, aes(koeff_levels,mean)) +
  ...
  scale_color_manual(name="Legende",values=c("black","black")) + 
  theme_bw() +
  guides(colour=guide_legend(override.aes=list(linetype=c("solid","blank"),shape=c(NA,16)))) +
