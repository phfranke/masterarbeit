...
## Kalibrierung
Das Modell verfügt über viele Parameter, welche zum Grossteil mit Werten aus der Literatur eingestellt werden können. Einzelne Faktoren sind durch die Literatur ungenügend abgedeckt und müssen über die Kalibration ermittelt werden. Dies betrifft die Wanderungen, welche wiederum durch mehrere Parameter bestimmt werden, unddie dichteabhängige Sterblichkeit. Die Wanderungen werden durch u.a. die Wandergeschwindigkeit und die Wanderdauer bestimmt. die Geschwindigkeit kann durch die Literatur bestimmt werden, die Wanderdauer hingegen ist nicht ganz klar. Im Rahmen der Kalibration werden deshlab folgende zwei Parameter ermittelt:

- die Dauer der Wanderungen
- der Koeffizient der dichteabhängigen Sterblichkeit

### Wanderdauer
Die Dauer der Wanderungen ist bestimmend für die Fähigkeit der Tiere neue Gewässer zu besiedeln. Es hat sich gezeigt, dass eine zu lange Wanderdauer die Anzahl besiedelter Weiher stark überhöht. Mit einer sehr kurzen Wanderdauer, wird kein neues Gewässer besiedelt. Beide Extremwerte entsprechen nicht der Beobachtung. Um das Modell besser an die Realität abzustimmen, wird deshalb ein erster Kalibrationsdurchlauf durchgeführt, mit welchen die Wanderdauer festgestellt wird. Der zweite unbekannte Parameter (der Dichtekoeffizienz) wird auf den Wert 0.1 festgelegt. Dieser experimentell ermittelte Wert stellt die funktionstauglichkeit des Modells sicher. Als beobachtete Grösse für diese Kalibration wird die Anzahl der besiedelten Gewässer verwendet.


### Dichteabhängige Sterblichkeit

Die dichteabhängige Sterblichkeit der Larven ist der einzige Parameter, welcher gänzlich unbekannt ist und durch Kalibration bestimmt werden muss. Für die Kalibration wird das Modell mehrere Male mit unterschiedlichen Parameterwerten durchgeführt und anhand der Resultate ein möglichst passender Wert für den Parameter bestimmt. Von den 
...
Die Anzahl besiedelte Gewässer sind stabil. Gemäss Aussgen der Projektleitung, werden Gewässer in der Umgebung regelmässig auf die Präsenz von Kammmolchen hin getestet. Wir gehen hier also davon aus, dass die Anzahl besetzte Gewässer zwischen den Jahren 2020 und 2022 stabil bei 5 lag. Dabei wird das Gewässer mit der Nummer BL195s mitgezählt. Hier sind die Dichten zwar so gering, dass die Bekämpfung eingestellt wurde, trotzdem wurden hier Kammmolche gesichtet.

Die Gewichtsverteilung ist nur mässig geeignet für die Kalibration, da das Gewicht der Kammmmolche nicht modelliert wird. Geht man davon aus, dass die Gewichstzunahme linear erfolgt, kann zwar die Gewichtsverteilung aus den Fangdaten mit der Altersverteilung aus dem Modell verglichen werden. Daraus aber einen geeigneten Parameterwert für die Sterblichkeit der Larven abzuleiten, erscheint nicht sinnvoll.
Die Anzahl gefangene Tiere sind aus zwei Gründen ebenfalls nur bedingt geeignet für die Kalibration:
- Die Daten aus dem Jahr 2021 sind nicht repräsentativ für die ganze Fortpflanzungssaison, da die Fangaktion mutmasslich zu spät begonnen wurde (siehe kapitel \@ref(daten)).
- Die Quote der gefangenen Tiere ist unbekannt und muss im Modell angenommen werden.
...
Im Modell wird dieser Koeffizient durch den Parameter `density_koeff` bestimmt. 
