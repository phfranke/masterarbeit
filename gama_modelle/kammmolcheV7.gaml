...
experiment dichte_larven {
	int anz_larven;
	int anz_larven_vorher <- 0;
	float sterb_larven;
	reflex larven_sterben {
		anz_larven <- sum(collect(where(nachwuchs,!each.ei),each.anzahl));
		sterb_larven <- anz_larven_vorher!=0 and anz_larven!=0 ? anz_larven_vorher / anz_larven : 1.0;
		anz_larven_vorher <- anz_larven;
	}
	output {
		display grafik {
			chart "larvendichte" type:series {
				datalist ["larvendichte"]
				color: [#black]
				value: [sterb_larven]
				marker: false;
			}
