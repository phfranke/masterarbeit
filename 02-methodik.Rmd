...
## Modellbeschreibung
In den folgenden Absätzen, wird die Funktionsweise des Modells beschrieben. Die Struktur folgt dem ODD-Protokoll wie von @grimm2006standard vorgeschlagen und später in @grimm2010odd erweitert. Die Bezeichnungen von Parametern und Variabeln hier entsprechen denjenigen im Modell und sind deshalb auf englisch.
...
- `migrant`: Wandernde Agenten, können von Agenten es den Spezies `juvenil` oder `adult` abstammen.
...
Das Modell berechnet die Anzahl Kammmolche im ganzen Modell und in den einzelnen Gewässern bei jedem Zeitschritt. In Abbildung \@ref(fig:grafik-prozess) sind die einzelnen Stadien ersichtlich. für die Übergange zwischen den Stadien, die Wanderungen und die Sterblichkeit sind spezifische Prozesse definiert, welche folgend erläutert werden. Einige Variabeln werden von Stadium zu Stadium weitervererbt. In den Prozessbeschrieben ist bei den Variabeln vermerkt, wer diese Eigenschaften vererbt. Die Prozesse sind hier in Pseudocode verfasst, damit lässt sich die technische Funktionsweise nachvollziehen ohne den Syntax der hier verwendeten Software kennen zu müssen.

```{r grafik-prozess, echo=FALSE,out.width="80%",fig.cap="Die schematische Darstellung zeigt die Stadien mit den spezifischen Prozessen"}
knitr::include_graphics("./bilder/grafik_prozesse.jpg")
```
...
Das Modell bildet natürlich, komplexe Propzesse nach. 
