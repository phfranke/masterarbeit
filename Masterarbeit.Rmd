...
### Agenten {#oddAgenten}
Das Modell bildet die Entwicklungszyklen, die Stadien der Kammmolche und spezifische Wanderbewegungen ab. Die Stadien werden in jeweils eigenen Spezies abgebildet, wobei die Stadien "Eier" und "Larven" zusammen in der Spezies "Nachwuchs" modelliert werden. Die Spezies "Migrant" dient der modellierung der Wanderbewegungen und kann Agenten der Spezies Juvenil und Adult aufnehmen. Für die Visualisierung der Umgebung und die Steuerung von Wanderbewegungen werden Daten zur Bodenbedeckung verwendet. Das Modell verfügt über folgende Spezies: 
...
#### Formelbasierte Simulationen des Nachwuchs, Spezies `offspring`
Die ersten beiden Stadien (Eier und Larven) sind an die Gewässer gebunden, umfassen sehr viele Individuen und sind von kurzer Lebensdauer. Sie unternehmen keine Wanderungen und erhalten im Modell lediglich die Aufgabe zu altern und zu sterben. Diese Stadien werden deshalb nicht nach einem agentenbasierten Modellansatz modelliert, sondern in einem formelbasierten. Das Modell verfolgt stark vereinfacht die Vorgehensweise, wie sie @WALLENTIN2017165 präsentiert. Die einzelnen Individuen werden demnach nicht als eigenständige Agenten geführt, sondern durch Attribute eines grösseren Agenten repräsentiert. Beim Eier legen (siehe Kapitel: \@ref(reproduction)) entsteht pro Weibchen ein Agent der Spezies `offspring`. Dieser beinhaltet die Anzahl Nachfahren im Attribut `count`. In der ganzen Lebensdauer der Agenten dieser Spezies, werden zwei Arten von Sterblichkeit berechnet. Diese verändern die Variabel `count` und sind im Detail in den Kapiteln \@ref(dichtesterb) und in @horner1985normal erklärt. Es sind folgende Ereignisse betroffen: 
...
### Modellvariabeln
Für die Steuerung der Agenten und für die Auswertung der Resultate werden zahlreiche Variabeln verwendet. Es wird unterschieden zwischen globalen Variabeln, welche aggregiert werden um Kalibration und Auswertungen vorzunehmen. 
...
- Zeitmanagement:
  - `starting_date` (Datum)
  - `end_date` (Datum)
  - `step = 60* 60* 24 Sekunden = 1 Tag`
  - `seasons` (Key-Value-Pairs)
  - `seasonNow` (Ganzzahl)
  - `current_date` (aktuelles Modelldatum)
- Anzahl Tiere:
  - `eggs_tot` (Ganzzahlen)
  - `larva_tot` (Ganzzahlen)
  - `juv_tot` (Ganzzahlen)
  - `adult_tot` (Ganzzahlen)
  - `migrantJuv_tot` (Ganzzahlen)
  - `migrantAdult_tot` (Ganzzahlen)
  - `occupiedPonds_count` (Ganzzahlen)
  - `catched_adult` (Liste adult)
  - `catched_juvenil` (Liste juvenil)

#### Variabeln pro Spezies
Bei Variabeln, welche nur zwei unterschiedlichen Werte annehmen können, wird jeweils ein Boolscher Datentyp (wahr/falsch) verwendet. Dies vereinfacht die Filterung der entsprechenden Individuuen. So werden weibliche Adulte mit folgenden  pseudcode gefiltert: `if isFemale`. Würde das Geschlecht mit dem Datentyp Text in der Variabel "gender" gespeichert, wäre die Filterung umständlicher zu schreiben (`if gender=="female"`), aufwändiger zu prozessieren und fehleranfälliger. Die Bezeichnung der betreffenden Variabeln zeigt welche Bedeutung die Variabeln annimmt, wenn sie wahr ist. Der andere mögliche Wert wird demnach durch den Wert falsch repräsentiert. Dies betrifft die Variabeln "istEi" (wahr=Ei, falsch=Larve), "istJuvenil" (wahr=juvenil, falsch=adult) und istWeiblich (wahr=weiblich, falsch=männlich). Andere Variabeln enthalten einen Verweis auf einen anderen Agenten. Dies ist der Fall bei der Variabel "heimischesGewässer". Hier wird ein Fremdschlüssel auf den entsprechenden Agenten der Spezies Bodenbedeckung gespeichert. Dies vereinfacht den Zugriff auf Variabeln der Brutgewässer über die Agenten, welche mit dem Gewässer verbunden sind. Folgende Variabeln werden auf Ebene Spezies festgelegt und ausgewertet:
...
### Prozesse
Das Modell berechnet die Anzahl Kammmolche insgesamt und in den Gewässern zu jedem Zeitpunkt. Dazu werden alle Entwicklungsstadien modelliert, Fortpflanzungen, Wanderungen und Todesfälle. Das Modell führt jeweils spezifische Prozesse durch, welche hier erläutert werden.

#### Fortpflanzung und Eier legen {#reproduction}
Weibchen legen Eier in das angestammte Gewässer. Dies ist nicht jederzeit möglich, es müssen eine Reihe von Bedingungen eintreffen, damit Eier abgelegt werden können:
- Die Jahreszeit: Fortpflanzung findet in einem festgelegten Zeitraum zwischen Februar und Mai statt.
- Es muss mindestens ein Männchen im gleichen Gewässer vorhanden sein wie das Weibchen.
- Seit der letzten Fortpflanzung ist die durch Parameter festgelegte Fortpflanzungspause verstrichen.

Beim Eier legen wird ein Agent der Spezies `offspring`  geschaffen. Dabei werden folgende Variabeln gesetzt:
...
#### Schlüpfen
Beim Schlüpfen werden drei Variabeln verändert:
- `count = count * survival_egg`
- `isEgg = false`
- `hatchDate_larva = current_date`

#### Dichteabhängige Sterblichkeit der Larven {#dichtesterb}
Im Larvenstadium wird eine dichteabhängige Sterblichkeitsrate eingeführt. Die Annahme ist, dass mit zunehmender Dichte die Sterblichkeit quadratisch ansteigt. Diese Kurve wird geeicht mittels eines Koeffizienten, dessen Höhe im Kalibrationsprozess bestimmt wird. Die grundlegende Sterblichkeit entspricht der Sterberate bei unbedeutender Larvendichte und wird anhand der Literatur angenommen. Die Sterblichkeit wird bei jedem Modellschritt, also täglich, angewendet. Die Sterblichkeitsrate bezieht sich jedoch auf die gesamte Lebensdauer. Deshalb wird die resultierende Sterblichkeit durch die Entwicklungsdauer in Tagen geteilt. Es resultiert die Formel \@ref(eq:sterblarve).

\begin{equation}
{Grundsterblichkeit}+(Koeffizient*Dichte)^{2}
(\#eq:sterblarve)
\end{equation}


#### Entwicklung von der Larven zum Juvenilen
Hier findet der Übergang vom formelbasierten zum individuenbasierten Paradigma statt. Beim erreichen des nötigen Alters (Parameter `maxAge_larva`) wird für jede Larve ein Agent der Spezies Juvenil geschaffen. Jeder Agent von `offspring` schafft also soviele Agenten von `juvenil`, wie die Variabel `count` zeigt. Dabei werden eine Reihe Eigenschaften vom kreierenden Agenten (ursprünglich von der Mutter) übernommen. Das Geschlecht wird nach einem zufallsmechanismus festgelegt.  Es werden folgende Variabeln gesetzt: 
...
### Initialisierung
Der Anfangszustands des Modells besteht aus der Bodenbedeckung und einer bestimmten Anzahl Kammmolche in einem bestimmten Gewässer. Die Bodenbedeckung wird aus einem Shape-File geladen und entsprechend ihrer Ausprägung eingefärbt. Über eine Attribut der Bodenbedeckung wird dasjenige Gewässer markiert, wo die Kammmolche sich zu Beginn befinden. Die Anzahl an Kammmolchen (Spezies `adult`) wird durch den Parameter `number_newts_start` festgelegt. Die Variabeln `location` und `nativePond` dieser Agenten werden auf diesen Standort gesetzt.  Während der Initialisierung wird ausserdem das Geschlecht vergeben und eine zufällige Altersspanne zwischen 0 und 500 Tagen festgelegt:
...
### Verwendete Daten, Bodenbedeckung
