...
experiment test {
	...
	float sterb_larve;
	float sterb_juvenil;
	reflex sterblichkeit {
		bool rewrite <- simulation.cycle=0;
		if current_date.day_of_year = 1 {
			ask simulation {
				myself.sterb_ei <- (anz_geschluepfte_larven != 0 and anz_gelegte_eier != 0) ? anz_geschluepfte_larven / anz_gelegte_eier : 0;
				myself.sterb_larve <- (anz_entw_juvenil != 0 and anz_geschluepfte_larven !=0) ? anz_entw_juvenil / anz_geschluepfte_larven : 0;
				myself.sterb_juvenil <- (anz_entw_adult !=0 and anz_entw_juvenil != 0) ? anz_entw_adult / anz_entw_juvenil : 0;
	//			float sterb_adult <- anz_adult / anz_adult_last; 
	//			float sterb_wanderer <- anz_wanderer / anz_wanderer_last;
				save [
					current_date,
					anz_gelegte_eier,
...
//			grid vernetzung transparency:0.5;			
		...
		monitor Sterblichkeit value: self.sterb_ei color: #white;  
	}
}