...
#### Emergenz {#emergenz}
Die Emergenz beschreibt Muster, welche sich durch die Summe der individuellen Entscheidungen ergeben. Die Emergenz wird durchzufällige Entscheidungen gefördert, da eher unerwartetes Verhalten auftritt. Durch komplett starre Regeln, wird weniger Emergenz möglich sein, sondern die Resultate werden immer gleich ausfallen. Das Modell hat zum Zweck die natürlichen Verbreitungsmuster der Kammmolche nachzubilden. Eine stark durch Zufall getriebenes Modell wäre allerdings chaotisch und die einzelnen Durchläufe würde sich so stark voneinander unterscheiden, dass eine Kalibrierung des Modells oder die Interpretation der Resultate verunmöglicht würde. Aus diesem Grund werden hier so viele Parameter wie möglich fest geschrieben und nur wenige Entscheidungen mit Zufallsmechanismen herbeigeführt. Viele Zufallsentscheidungen nivellieren sich über die grosse Anzahl Agenten und über die lange Dauer des Modells aus.


Wesentliche Kennzahlen der Verbreitung von Italienischen Kammmolchen im Projektgebiet sind die Anzahl Tiere sowie die besiedelten Gewässer. Das Verbreitungsgebiet wird durch die besiedelten Gewässer bestimmt. Das wesentliche Muster, welches in diesem Modell repliziert werden soll, ist demnach hauptsächlich die besiedelten Gewässer.




In natürlichen Prozessen verfügen alle Entscheidungen über gewisse Schwankungsbereiche. Über viele Individuen und längere Zeit ergeben sich aber eindeutige Muster, welche aus der Summe aller Entscheidungen resultieren und nicht unbedingt nur mit einzelnen Faktoren erklärbar sein müssen.

Dieses Modell versucht das natürliche Verbreitungsmuster der Italienischen Kammmmolche zu ergründen und nachzubilden ohne dabei die individuellen Spielräume exakt abzubilden. Dies würde die Gefahr beinhalten, dass das Modell chaotisch reagiert und die Resultate sehr grosse Schwankungen aufweist. Dadurch wäre eine Kalibrierung des Modells und die Interpretation der Daten sehr schwierig wenn nicht sogar unmöglich.
...
Das Modell kann aufgrund dieser Ergebnisse **nicht** als valide eingestuft werden. Dies ist nicht zwingend darauf zurückzuführe, dass die grundsätzliche Mechanik im Modell nicht korrekt ist, sondern kann mehrere Ursachen haben:

1. Die Kalibration ist nicht korrekt
2. Der Mechanismus wie im Modell Tiere gefangen und gezählt werden, ist nicht korrekt
3. Die Fortpflanzung, Wanderung und Sterblichkeit ist nicht adäquat abgebildet und es entstehen zu wenige Kammmolche

__Ja nachdem was die qualitative Validierung ergibt, fällt Punkt 2 oder 3 weg.__
...
Im ersten Fangjahr werden, vor allem in den Szenarien mit den höheren Fangwahrscheinlichkeiten, die tatsächlichen Fänge sehr gut widergegeben. Das Modell ist also korrekt kalibriert. Die Fangzahlen aus den Jahren 2021 und 2022 gehen offensichtlich sehr stark auseinander. Bei der Betrachtung der Fangzahlen auf der Zeitachse (siehe Abbildung \@ref(fig:val3)), fallen die regelmässigen Schwankungen ins Auge. Die Fangzahlen brechen in regelmässig wirkenden Abständen stark ein und erholen sich innert einiger Jahre wieder. Es entstehen zyklische Schwankungen mit einer Frequenz zwischen fünf und zehn Jahren. Hier sind die Fänge aus allen Modelldurchläufen mit der Fangwahrscheinlichkeit `80%` und `90%` zusammengerechnet. Die ausgezogene Linie zeigt den Mittelwert und der graue Bereich die Spanne der gefangenen, adulten Kammmolche pro Jahr.

```{r val3, echo=FALSE, fig.cap="Zeitreihe der Fangzahlen in den Fangszenarien 80% und 95%",out.width="60%"}
graph_val_year

```

Auffällig in der Darstellung der zeitlichen Verläufe sind neben dem zyklischen Schwankungen ist auch die Tatsache, dass Spanne zwischen Minimal- und Maximalwert erstaunlich gering ausfällt. Die spitzen der Schwankungen laufen sehr spitz zu, was darauf hindeutet, dass in allen Modelldurchläufen die Spitzen im gleichen Zeitraum auftreten. 
...
Die Grafiken in Abbildung \@ref(fig:val-alle) zeigen die Anzahl adulter Kammmolche in sämtlich Modelldurchläufen als separate Linie. In der linken Grafik sind die Resultate aus der ersten Modellversion abgegbildet, in welcher nur in bestimmten Gewässern bekämpft wird. Die rechte Grafik bildet die Resultate aus der zweiten Version ab, wo in allen Gewässer Kammmolche abgefangen werden. Durch die separate Darstellung der einzelnen Modelldurchläufe kann noch besser abgeschätzt werden, ob die Zyklen zufällig entstehen oder auf eine Mechanik im Modell zurückzuführen sind. In ausnahmslos allen Durchläufen gibt es Spitzen in den beiden Jahren 2016 und 2021. Das ist noch bevor die Bekämpfung gestartet wird und beide Modellversionen die gleiche Voraussetzung haben. Die Modellversion mit der verstärkten Bekämpfung (rechte Grafik) zeigt auch über den restlichen Modellraum sehr ausgeprägte Spitzen. In der ersten Modellversion verlaufen sich diese Spitzen. Wenn einzelne Linien betrachtet werden, sind aber auch dort die zyklischen Schwankungen sichtbar.

Die Fangzahlen in Tab. \@ref(tab:tablecatches) deuten nicht darauf hin, dass im realen Ökosystem ebenfalls ähnliche Schwankungen auftreten. Obwohl hier nur Daten aus zwei Jahren vorliegen, muss davon ausgegangen werden, dass die Population keine solche Schwankungen aufweist.

Das Modell beginnt aus ungeklärten Gründen zu schwingen. Da die Daten für die Kalibration und die Validierung sehr nahe beieinander liegen, beeinträchtigt das Schwingverhalten die Validierung. Auffällig ist auch, dass die Schwankungen über alle Modellläufe recht regelmässig aufzutreten scheinen. Diese Schwankungen werden auch durch die Resultate in den Abbildungen \@ref(fig:res) und \@ref(fig:res2) bestätigt. Die Anzahl adulter Kammmolche schwankt ebenfalls über alle Modellläufe in einer erstaunlichen Regelmässigkeit. So weist die Kammmolchpopulation in allen Grafiken ein Spitze in den Jahren 2016 und 2021 auf. Diese liegen zeitlich noch bevor die Bekämpfung beginnt, es kann also davon ausgegangen werden, dass das Modell grundsätzlich zu Schwankungen neigt und nicht durch den Fangmechanismus ausgelöst wird.

Daraus kann gefolgert werden, dass das oszillierende Verhalten des Modells die Validität beeinträchtigt. Warum das Modell zu diesem Verhalten neigt, kann hier nicht eruiert werden. Darüber hinaus kann nicht abschliessend geklärt werden, ob die grundsätzliche Funktionsweise des Modells valide ist oder ob zusätzlich zu den Schwingungen weitere Beeinträchtigungen bestehen.


## Biologische Aussagen
Es kann nicht genügend nachgewiesen werden, dass das Modell die realen Verhältnisse korrekt wiedergibt. Dies wird hauptsächlich auf unerklärtes Schwingverhalten zurückgeführt werden. Dennoch ist es die Aufgabe dieser Arbeit, so viele Aussagen über die Wirksamkeit der Bekämpfungsmethoden im Mühlebachtal zu tätigen wie möglich.
